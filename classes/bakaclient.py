...
class BakaClient:
    ...
    def get_marks(self, final=False):
        """Fetches the marks from the API."""
        data = fetch_from_api(self.token_data, self.auth_data, f'/marks{"/final" if final else ""}')
        return data

    def get_changes(self):
        """Fetches the changes from the API."""
        data = fetch_from_api(self.token_data, self.auth_data, '/substitutions')
        return data
